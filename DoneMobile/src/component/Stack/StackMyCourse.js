...
import { Trophy,Sad } from '../../assets/index'
    ...
    render() {
        return (
            ...
            <FlatList
                ...
                keyExtractor={item => item.subtopics.subTopicId}
                renderItem={({ item, index }) => {
                    return (
...
class ExamScreen extends Component {
    ...
    handleResult(results){
        ...
        var pass = false;
        results.forEach(e => {
            if(e.response === e.answer){
                count += 1;
            }
        });
        pass = (count >= (length/2)) ? true : false;
        result = count + "/"+length;
        console.log(pass); 
        console.log(result);
...
class ResultExam extends Component{
    render(){
        const ketqua = (this.props.route.params.pass) ? "Dau" : "Rot";
        if(ketqua == "Dau"){
            return (
                <>
            <View style={styles.containerResult}>
            <Image style={styles.imgResult} source={Trophy}/>
            <Text style={{fontSize:25,fontWeight:'bold',color:'#00cf07'}}>You Pass The Exam</Text>
            <Text style={{fontSize:25,fontWeight:'bold'}}>Point: {this.props.route.params.result}</Text>
            <TouchableOpacity style={styles.btnSubmitNow} onPress={() => this.props.navigation.pop(2)}>
                <Text style={styles.txtSubmitNow}>Submit!!!</Text>
            </TouchableOpacity>
            </View>
            </>
            )}
        ...
        return(
            ...
            <>
           <View style={styles.containerResult}>
            <Image style={styles.imgResult} source={Sad}/>
            <Text style={{fontSize:25,fontWeight:'bold',color:'red'}}>Sorry!You fail the exam</Text>
            <Text style={{fontSize:25,fontWeight:'bold'}}>Point: {this.props.route.params.result}</Text>
