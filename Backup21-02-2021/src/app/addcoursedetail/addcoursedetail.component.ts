...
export class AddcoursedetailComponent implements OnInit {
  ...
  openQuizDialog(): void {
    const dialogRef = this.dialog.open(QuizdialogComponent, {
      width: '400px',
      data: {quizName: this.QuizName,createdDate: this.CreatedDate,lastEdited: this.LastEdited,hastag:this.QuizHastag,thumbnailImage: this.QuizThumbnailImage,quizCode: this.QuizCode,lessonId: this.LessonId,lessondataset: this.lessondataset }
    });
    dialogRef.afterClosed().subscribe(result => {
      console.log(result);
      this.createNewTopic(result);
