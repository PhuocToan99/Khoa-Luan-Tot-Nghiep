...
export class QuestionpooldetailComponent implements OnInit {
  ...
  constructor(private excelService:ExcelService,private route: ActivatedRoute,private quizService: QuizService, private router: Router,private choiceService:ChoiceService,
    public dialog: MatDialog,private notificationService: NotificationService,private examQuizService: ExamquizserviceService) {
    this.route.params
    .subscribe(
      (params: Params) => {
        this.id = +params['id'];
    ...
    dialogRef.afterClosed().subscribe(result =>{
      ...
      if(result != null && result != undefined){
        ...
        try{
          ...
          this.quizService.updateQuiz(quiz.quizId,quizResult);
          this.editChoice(quiz.choices[0].choiceId,result.option1,result.imageOption1,result.imageOption1,quiz.quizId,result.isCorrect,"1");
          this.editChoice(quiz.choices[1].choiceId,result.option2,result.imageOption2,result.imageOption2,quiz.quizId,result.isCorrect,"2");
          if(result.option3 != "" || result.imageOption3 != undefined || result.imageURLOption3 != ""){
            if(quiz.choices[2] == undefined){
              this.addNewAnwser(result.option3,result.imageOption3,result.imageOption3,quiz.quizId,result.isCorrect,"3");
            }
            else{
              this.editChoice(quiz.choices[2].choiceId,result.option3,result.imageOption3,result.imageOption3,quiz.quizId,result.isCorrect,"3");
            }
          }
          if(result.option3 == "" && result.imageOption3 == undefined && result.imageURLOption3 == ""){
            if(quiz.choices[2] != undefined){
              this.choiceService.deletechoice(quiz.choices[2].choiceId);
            }
          }
          if(result.option4 != "" || result.imageOption4 != undefined || result.imageURLOption4 != ""){
            if(quiz.choices[3] == undefined){
              this.addNewAnwser(result.option4,result.imageOption4,result.imageOption4,quiz.quizId,result.isCorrect,"4");
            }
            else{
              this.editChoice(quiz.choices[3].choiceId,result.option4,result.imageOption4,result.imageOption4,quiz.quizId,result.isCorrect,"4");
            }
          }
          if(result.option4 == "" && result.imageOption4 == undefined && result.imageURLOption4 == ""){
            if(quiz.choices[3] != undefined){
              this.choiceService.deletechoice(quiz.choices[3].choiceId);
            }
          }
          if(result.option5 != "" || result.imageOption5 != undefined || result.imageURLOption5 != ""){
            if(quiz.choices[4] == undefined){
              this.addNewAnwser(result.option5,result.imageOption5,result.imageOption5,quiz.quizId,result.isCorrect,"5");
            }
            else{
              this.editChoice(quiz.choices[4].choiceId,result.option5,result.imageOption5,result.imageOption5,quiz.quizId,result.isCorrect,"5");
            }
          }
          if(result.option5 == "" && result.imageOption5 == undefined && result.imageURLOption5 == ""){
            if(quiz.choices[4] != undefined){
              this.choiceService.deletechoice(quiz.choices[4].choiceId);
            }
          }
  ...
  showOptions(event:any,quiz:Quiz): void {
    ...
    if(event == true){
      ...
      examquiz.examQuizId = quiz.quizId;
