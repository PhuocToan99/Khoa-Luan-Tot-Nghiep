...
import { TopicDialogData,QuizDialogData } from '../dialog/shared/sharedata';
...
export class AddcoursedetailComponent implements OnInit {
  ...
  public QuizHastag:string ='';
  public QuizCode: string='';
  public LessonId: string='';
  public QuizThumbnailImage;
  ...
  private courseService: CourseService, private userservice: UserService, private imageLoadService: ImageloadService,private quizservice: QuizService) {
  ...
  openSubtopicDialog1(subtopic: Subtopic): void {
    const dialogRef = this.dialog.open(SubtopicdialogComponent, {
      width: '400px',
      data: { subTopicId: subtopic.subTopicId, subTopicTitle: subtopic.subTopicTitle, subTopicNumber: subtopic.subTopicNumber }
    });
  ...
  openLessonDialog1(lesson: Lesson): void {
    const dialogRef = this.dialog.open(LessondialogComponent, {
      width: '400px',
      data: { lessonId: lesson.lessonId, lessonTitle: lesson.lessonTitle, lessonContent: lesson.lessonContent, subTopicId: lesson.subTopicId }
    });
  ...
  openQuizDialog(): void {
    ...
    const dialogRef = this.dialog.open(QuizdialogComponent, {
      width: '400px',
      data: {quizName: this.QuizName,createdDate: this.CreatedDate,lastEdited: this.LastEdited,hastag:this.QuizHastag,thumbnailImage: this.QuizThumbnailImage,quizCode: this.QuizCode,lessonId: this.LessonId,courseId:this.CourseId }
    });
    dialogRef.afterClosed().subscribe(result => {
      result.createdDate = this.LastUpdate;
      console.log(result);
      this.createNewQuiz(result);
    });
  }
  public createNewQuiz = async (quiz) => {
    try {
      let quiznew = new Quiz();
      quiznew.courseId = quiz.courseId;
      quiznew.createdDate = quiz.createdDate;
      quiznew.lastEdited = quiz.createdDate;
      quiznew.hastag = quiz.hastag;
      quiznew.lessonId = quiz.lessonId;
      //quiznew.quizCode = quiz.quizCode;
      quiznew.quizName = quiz.quizName;
      quiznew.thumbnailImage = null;
      const result: any = await this.quizservice.postquiz(quiznew);
      console.log(result);
      alert('Add sucessfully');
      this.loadpage();
    }
