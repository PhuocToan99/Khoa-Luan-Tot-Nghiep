...
export class AddcoursedetailComponent implements OnInit {
  ...
  openDialog(): void {
    const dialogRef = this.dialog.open(CoursedialogComponent, {
      width: '400px',
      data: {
        courseId: this.coursedata.courseId, courseName: this.CourseName, courseDuration: this.CourseDuration, description: this.Description, startDate: this.StartDate, lastUpdate: this.LastUpdate, hastag: this.CourseHastag, level: this.CourseLevel, thumbnailImage: this.ThumbnailImage, rating: this.Rating, numberOfVoters: this.NumberOfVoters,
  ...
  async blockStateChange(quizCode){
    ...
    list.forEach(async e => {if(e.examCode == quizCode){
